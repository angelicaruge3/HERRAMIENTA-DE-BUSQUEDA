...
class VentanaPrincipal(QMainWindow):
    ...
    def realizar_busqueda(self):
        # Obtener el nombre del autor ingresado por el usuario
        nombre_autor = self.caja_busqueda.text()
        ...
        if resultados:
            for resultado in resultados:
                self.resultados_busqueda.append(f"Título: {resultado[1]}")
                self.resultados_busqueda.append(f"Autor: {resultado[2]}")
                self.resultados_busqueda.append(f"Editorial: {resultado[3]}")
